...
if st.session_state.upload:
    ...
    with ((((t4)))):
        ...
        with t41:
            ...
            def filter_dataframe(filtered_df, checked_columns, columns_to_check_for_duplicates,columns_without_reimbursement_id, dfholiday, filename):
                    ...
                    if 'Holiday Transactions' in checked_columns:
                        ...
                        if len(checked_columns) == 1:
                            ...
                            filtered_df = filtered_df[filtered_df['Invoice Number'].isin(similar_invoices)]
                                ...
                                item in checked_columns for item in ['Inv-Special Character', 'Holiday Transactions']):
                            filtered_df['Document Date'] = pd.to_datetime(filtered_df['Document Date'],
                                                                          format='%Y/%m/%d', errors='coerce')
                            filtered_df['Posting Date'] = pd.to_datetime(filtered_df['Posting Date'], format='%Y/%m/%d',
                                                                         errors='coerce')
                            ...
                            dfholiday['date'] = pd.to_datetime(dfholiday['date'], format='%Y/%m/%d', errors='coerce')
                            filtered_df = filtered_df[filtered_df['Posting Date'].isin(dfholiday['date'])]
                            # columns_without_reimbursement_id = [col for col in columns_to_check_for_duplicates if
                            #                                     col != 'Reimbursement ID']
                            filtered_df = filtered_df[~filtered_df.duplicated(subset='Invoice Number', keep='last')]
                            # Create a list of tuples with original and cleaned invoice numbers
                            grouped = filtered_df.groupby('Reimbursement ID')
                            similar_invoices = set()
                        ...
                        # Filter the dataframe to include only similar invoices
                        filtered_df = filtered_df[filtered_df['Invoice Number'].isin(similar_invoices)]
                    elif len(checked_columns) >= 2 and 'Inv-Special Character' in checked_columns and ['Holiday Transactions','Reimbursement ID']not in checked_columns:
                ...
                # Sorting logic
                    ...
                    try:
                        return filtered_df, checked_columns, columns_to_check_for_duplicates, dfholiday, filename, columns_without_reimbursement_id
                    except Exception as e:
                        st.error(f'An error occurred: {e}')
                        return None, None, None, None, None, None
            filename = "Exceptions.xlsx"
            filtered_df, checked_columns,columns_without_reimbursement_id, columns_to_check_for_duplicates, dfholiday, filename = filter_dataframe(
                    filtered_df, checked_columns, columns_to_check_for_duplicates, dfholiday, filename,columns_without_reimbursement_id
                )
