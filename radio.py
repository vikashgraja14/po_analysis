...
if st.session_state.upload:
    ...
    display_trends(filtered_df)

    # @st.experimental_fragment
    def display_exceptions(data):
        with t3:
            ...
            def filter_dataframe(filtered_df, checked_columns, columns_to_check_for_duplicates, dfholiday, filename):
                if not checked_columns:
                    ...
                    if 'Holiday Transactions' in checked_columns:
                        if len(checked_columns) == 1 and 'Holiday Transactions' in checked_columns:
                            ...
                            filename = "Holiday Transactions with 70% SIMILAR INVOICE.xlsx"


                        elif '70% SIMILAR INVOICE' not in checked_columns:
                            filtered_df = filtered_df[filtered_df['Posting Date'].isin(dfholiday['date'])]
                            ...
                            columns_str = '_'.join(columns_to_check_for_duplicates)
                            filename = f"Holiday Transactions with same Column VAlues.xlsx"
                            ...
                            if 'Reimbursement ID' not in checked_columns and 'Cost Center' not in checked_columns:
                                sort_columns = checked_columns + ['Reimbursement ID', 'Cost Center']
                            elif 'Reimbursement ID' in checked_columns and 'Cost Center' not in checked_columns:
                                sort_columns = checked_columns + ['Cost Center']
                            elif 'Cost Center' in checked_columns and 'Reimbursement ID' not in checked_columns:
                                sort_columns = checked_columns + ['Reimbursement ID']
                            else:
                                sort_columns = checked_columns
                            filtered_df['Posting Date'] = filtered_df['Posting Date'].dt.date
                            filtered_df['Document Date'] = filtered_df['Document Date'].dt.date
                            filtered_df['Verified on'] = filtered_df['Verified on'].dt.date
                            ...
                            filename = f" Transactions with same column.xlsx"
                    if '70% SIMILAR INVOICE' in checked_columns and 'Holiday Transactions' not in checked_columns:
                        filtered_df = filtered_df[
                            filtered_df.duplicated(subset=columns_to_check_for_duplicates, keep=False)]
                        filtered_df['Is Similar'] = False
                        for i in range(len(filtered_df)):
                        ...
                        columns_str = '_'.join(columns_to_check_for_duplicates)
                        if 'Holiday Transactions' in checked_columns:
                                sort_columns = checked_columns + ['Posting Date', 'Reimbursement ID', 'Cost Center',
                                                                  'Amount']
                                filtered_df = filtered_df.sort_values(by=sort_columns)
                                filtered_df.reset_index(drop=True, inplace=True)
                                filtered_df.index += 1
                        elif 'Reimbursement ID' not in checked_columns and 'Cost Center' not in checked_columns:
                                sort_columns = checked_columns + ['Reimbursement ID', 'Cost Center']
                                filtered_df = filtered_df.sort_values(by=sort_columns)
                                filtered_df.reset_index(drop=True, inplace=True)
                                filtered_df.index += 1
                        ...
                        elif '70% SIMILAR INVOICE' in checked_columns:
                                sort_columns = checked_columns + ['Posting Date', 'Reimbursement ID', 'Cost Center',
                                                                  'Amount']
                                filtered_df = filtered_df.sort_values(by=sort_columns)
                                filtered_df.reset_index(drop=True, inplace=True)
                        ...
                        filename = f" Transactions with same values.xlsx"
                return filtered_df, ', '.join(checked_columns), columns_to_check_for_duplicates, dfholiday, filename
